...
There are different types of moving data based on source and destination. Here we look at each with an example:
...
1. Placing *$* before a label name = Getting the memory address of the variable.
	...
	Example:  
	Moving the value in *location* variable to register EDI.
...
2. Putting parentheses around a register = Pointing to the address which the register contains.
	...
	Example:  
	Moving value 9 in the memory location pointed by EDI register.

	`movl $9, (%edi)`

	Example:  
	Moving value 9 in the memory location pointed by EDI + 4.

	`movl $9, 4(%edi)`

	Example:  
	Moving value 9 in the memory location pointed by EDI – 2.
